...
import org.apache.beam.sdk.io.hdfs.Write;
...
public class HdfsWordCount {
    ...
    public static void main(String[] args) throws IOException {
        ...
        PCollection<String> records = p.apply(Read.from(source));

        HDFSFileSink<String, NullWritable, Text> sink =
                HDFSFileSink.toText(options.getOutput());

        // Concepts #2 and #3: Our pipeline applies the composite CountWords transform,
        // and passes the static FormatAsTextFn() to the ParDo transform.
        records.apply(new CountWords())
                .apply(MapElements.via(new FormatAsTextFn()))
                .apply("WriteCounts", Write.to(sink));
